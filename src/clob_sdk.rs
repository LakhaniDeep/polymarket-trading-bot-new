...
pub fn check_private_key() -> Result<()> {
    let lib = load_lib()?;
    let f: libloading::Symbol<
        unsafe extern "C" fn(
            *mut c_char,
    ...
    > = unsafe { lib.get(b"clob_sdk_check_private_key") }
        .context("clob_sdk_check_private_key not found in CLOB SDK library")?;
    ...
    if ret != 0 {
        ...
        let msg = std::str::from_utf8(unsafe { std::slice::from_raw_parts(err_buf.as_ptr() as *const u8, len) })
            .unwrap_or("unknown error");
        anyhow::bail!("check_private_key failed: {}", msg);
    }
    Ok(())
}